'''This is a cleaned up version of RayTraceFunCleaner.py. It has also been shortened to ONLY include the functions that are necessary, rather than ones that were used when building this. This now includes Zhaodi's suggestions and a few new functions, as well as more removed. Mira'''
import numpy as np
import numpy
...
'''Give ray and everything in global, does work in local, returns in global: ORIGINAL NOT INCLUDING RANGE'''
...
def IntM2(Ray,coeffmirr,originmirr):
    ...
    if SRM(intpoint,coeffmirr,originmirr) == True:
        ...
        return
''' makes  a circle around center c with radius r'''
def circle(c,r):
