...
class App:
    ...
    def __init__(self, parent):
        ...
        self.www = Label(parent, text = "Plot Number:", fg = "blue")
        self.www.pack()
        
        self.Number = Entry(parent, textvariable = self.j)
        self.Number.pack()
    ...
    def run(self):
        ...
        self.J = self.j.get()
        if self.Analysis == 'on':
            self.xint, self.yint = d['delay' + self.t], d['sig' + self.t]
            ...
            self.xspec, self.yspec = 300*Nu,np.abs(S)
        ...
        if self.Interferogram == 'on' and self.Spectrum == 'on':
            ...
            if self.Interferogram == 'on':
                ...
                pl.title(str(d['run']))
                line, = pl.plot (self.xint, self.yint, 'r', linewidth = .5)
                pl.xlim(-35,35)
                ...
                x = np.genfromtxt('../../data/raw_data/xref.txt')
                y = np.genfromtxt('../../data/raw_data/yref.txt')
                fig = pl.figure(self.J)
                #pl.plot (x,y)
                line, = pl.plot (x,y, 'b' , linewidth = .5)
                #pl.xlim(-35,35)
                h = 'Reference'
                line.set_label(h)
                pl.legend()
            ...
            if self.Spectrum == 'on':
                ...
                pl.title(str(d['run']))
                line, = pl.plot (self.xspec, self.yspec, 'r')
                pl.xlim(50,750)
                m = self.time + ' Spectrum'
                M = str(m)
                line.set_label(M)
                
                x1 = np.genfromtxt('../../data/raw_data/xrefspectrum.txt')
                y1 = np.genfromtxt('../../data/raw_data/yrefspectrum.txt')
                fig = pl.figure(self.J)
                #pl.plot(x1,y1)
                line, = pl.plot(x1,y1)
                pl.xlim(50,750)
