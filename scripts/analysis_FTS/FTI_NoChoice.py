...
class App:
    ...
    def __init__(self, parent):
        ...
        self.Number = Entry(In, textvariable = self.j, width = 5)
        ...
        self.SetColor = Entry(In, textvariable = self.color, width = 5)
    ...
    def run(self):
        ...
        if self.Analysis == 'on':
            ...
            self.v=(d['speed'])
            X = self.v*T #full distance
            dx = dt*self.v #smallest amount of distance travelled
            self.total_t = (d['scan time']) #how long it ran
            ...
            self.xint, self.yint = t, c

            self.xspec, self.yspec =300*fFull, np.abs(np.fft.rfft(c))/k_v*tau
        ...
        if self.Interferogram == 'on' and self.Spectrum == 'on':
            ...
            figattr[2].plot (self.xspec, np.abs(H))
            ...
            s = 'sample frequency: %5.0f\r\nvelocity: %2.1f\r\ntime: %2.1f'% ((d['sample freq']), self.v, self.total_t)
    ...
    def window(self): 
        ...
        label4 = Label(top, fg = 'blue', text = 'Deconvolution Tests')
        label4.pack()
        
        Decon_Spec = Checkbutton(top, text = 'Compare Interferograms', variable = self.U, command = self.compint)
        Decon_Spec.pack(side = TOP)
    ...
    def Calculate(self):
        ...
        if self.COMPINT == 'on':
            L1=np.sum(np.abs(self.AAA))
            pl.plot(self.ttt,self.AAA/L1, label = 'output')
            L2 = np.sum(np.abs(self.ccc))
            pl.plot(self.ttt, self.ccc/L2, label = 'deconvolved')
            pl.legend()
            pl.show()
        if self.ratio == 'on':
            self.k = IntVar()
