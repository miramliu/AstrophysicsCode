...
This routine runs an FTS scan only
User requests number of scans, maximum distance and scan speed
...
def fts_scan_zc(niter,max_d,speed,ser):
	"""
		xxx name		run name and will be the name of the pkl file
		...
		xxx max_nu		maximum desired frequency (GHz)
		xxx oversample	adc sample rate multiplier over what is needed for max_nu
		xxx gain		ADC gain 0 => +10V to -10V, 1 is x10, 2 is x100, 3 is x1000
		ser		serial port handle for zaber stage
		xxx lj_hand		handle for connection to ADC
	...
	for iiter in np.arange(niter):
		...
		time.sleep(1)		#give it an extra 1s to finish moving
		zc.move_vel(ser,-speed)		#move reverse at command speed
		time.sleep(1)		#give it an extra 1s to finish moving
	#
	# clear digital outputs move to the wlf, park
