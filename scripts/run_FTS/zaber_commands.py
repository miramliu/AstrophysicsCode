...
#Position of white light fringe in optical mm far, depends on serial number of device being used
white_light_fringe_dict = {}
white_light_fringe_dict[44248] = 113.58 #117.
white_light_fringe_dict[43469] = 112.776
...
def init_zaber(dev):#initialze the serial port, return port handle
    ...
    if ser.isOpen():
        ...
        #checking serial number to assign correct white light fringe value
        err,ser_num = get_device_ser(ser)
        if ser_num in white_light_fringe_dict:
            global white_light_fringe
            white_light_fringe = white_light_fringe_dict[ser_num]
            print 'Device serial number: ' + str(ser_num)
        else:
            raise ValueError('No white light fringe value for this serial number')
        err = 0
    else:
...
def get_device_ser(port):# Get the device serial number
    err,response = output_command(port,'/1 get system.serial\r\n')
    ser = int(response[17:23])
    ser_str = ser
    return err,ser_str
