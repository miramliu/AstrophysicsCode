'''putting it all into one file and for tilting things / testing out misalignments Mira'''
import numpy as np
import numpy
...
#from PossiblePathsTilting import *
...
origin7 = [128.3,7.5,39.850000]
origin8 = [-96.225,-120.50,0.]
origin9 = [-96.225,120.50,0.]
origin10 = [-128.3,7.5,-39.850000]

#coefficients are like [a,b,c]
...
#these are the functions above but edited to include position of mirror
'''output from detector. GIVE initial RAYS AND Y position, returns output rays from detector if mirror at Y'''
def OFDM(Rays,y): 
    Rayf = []
...
'''GIVE initial RAYS AND Y position, returns output rays from detector if mirror at Y'''
def OFDMtilt(Rays,y,thetmirr): 
    Rayf = []
    for i in range(len(Rays)):
        ...
        for j in range(8):
            origin = (0,y,0)
            out = Paths[j](Ri,p1,p2,p3,p4,origin,thetmirr)
            if out is not None:
                Rayf.append(out)
    return Rayf
...
''' give rays to be run through this FTS at a specific y, returns the good rays and the region. not used anymore'''
def RunRaysMtilt(Rays,y,thetmirr): #just give number of rays to be run through this FTS at a specific y!
    Rayf = OFDMtilt(Rays,y,thetmirr)
    G= checkoutraysM(Rayf,[160.375,-113],7.9375) # GOOD RAYS ONLY 
    Gtestsorted = sortgrid(G)
    return Gtestsorted
...
def RunOneRayTiltSource(Lamd,Nsize,spo,thetmirr): 
    n = 1
    r = 0
    Rays = makeraysVERTICAL(spo,r,n)
    jx,jy = gridlines(7.9375,[160.375,-113],200) #these are now the PIXELS
    ...
    for y in np.linspace(-18,18,int(Nsize)):
        PTot=0
        OutRays=RunRaysMtilt(Rays,y,thetmirr) #eight each
        for j in range(len(Pix)): #per PIXEL
            Ex4i = 0 #adding PER PIXEL from parts of RAYS in this PIXEL
            Ey4i = 0 #THIS IS WHERE THEY WILL INTERFERE
            for i in range(len(OutRays)): #per ray IN THIS PIXEL
...
def ReflEll(Ray,thetL,originL,coeffellipse,center,ranges):
    ...
    pointsf,vectsf = REPCNi(coeffellipse,SPLi,VPLi) #IN LOCAL COORDINATES, REFLECTION OFF ELLIPSOID
    if SR3B(ranges, pointsf[0],pointsf[1],pointsf[2], center) == True:
        SPLf,VPLf = RT(pointsf,vectsf,thetL,originL,thetG,originG) #IN GLOBAL COORDINATES, 
        Ray_Refl.append(Ray[0] + np.pi)
        Ray_Refl.append(Ray[1])
        Ray_Refl.append(SPLf)
        Ray_Refl.append(VPLf)
...
def spab(a,b):
    ...
    diffa=4-a
    diffb=4-b
    sp[0]=sp[0]-int(diffb)*2.5
    sp[1]=sp[1]-int(diffa)*2.5
    return sp
